...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Surat Keluar / ROYA</li>
@endsection
    ...
    <div class="card">
        <div class="card-body">
            ...
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            <button type="button" class="btn btn-success mb-2" data-toggle="modal" data-target="#roya_modal"><i class="fas fa-file-download"></i> Import Excel</button>
            @endcan
            <table id="roya" class="table table-bordered table-striped table-hover table-responsive roya table-sm">
                <thead>
                    <th>NO</th>
                    <th>NOMOR SURAT</th>
                    <th>PERIHAL</th>
            ...
            <!-- Modal Import Excel -->
            <div class="modal fade" id="roya_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form id="importForm" action="{{ route('roya.import') }}" method="POST" enctype="multipart/form-data">
                        <div class="mt-3 mx-3">
                            <label for="file" class="control-label">Download Template Excel</label>
                            <a href="{{ route('roya.export') }}" class="btn btn-secondary d-block"><i class="fas fa-file-upload"></i> Template Excel</a>
                        </div>
...
@includeIf('surat_keluar.roya.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#roya").DataTable({
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/surat_keluar/roya') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/surat_keluar/roya') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#roya_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data ROYA');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data ROYA');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data ROYA');
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data ROYA');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("roya.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {
                    ...
                    // Tutup modal
                    $('#roya_modal').modal('hide');

                    // Reset form
                    $('#importForm')[0].reset();

                    // Reload DataTable
                    $('#roya').DataTable().ajax.reload();
                }
            },
            error: function (xhr) {
                // Jika gagal
    ...
    var fullImageUrl = '{{ asset('storage/surat_keluar/roya') }}/' + imageUrl;
