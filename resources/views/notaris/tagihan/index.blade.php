...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Notaris / Tagihan</li>
@endsection
    ...
    <div class="card">
        <div class="card-body">
            ...
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            @endcan
            <button id="toggleKeterangan" class="btn btn-warning mb-2">Tampilkan Keterangan</button>
            <table id="tagihan" class="table table-bordered table-striped table-hover table-responsive tagihan table-sm">
                <thead class="text-center">
                    <tr>
                      <th rowspan="2">NO</th>
                      <th rowspan="2">TANGGAL BAYAR</th>
                      <th rowspan="2">BULAN TAGIHAN</th>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="imageModalLabel">FILE</h5>
...
@includeIf('notaris.tagihan.form')
@endsection
...
// Variabel global untuk status toggle
let showFullKeterangan = false;

// Event listener untuk tombol toggle
document.getElementById('toggleKeterangan').addEventListener('click', function () {
    showFullKeterangan = !showFullKeterangan;

    // Perbarui teks tombol
    this.textContent = showFullKeterangan ? 'Sembunyikan Keterangan' : 'Tampilkan Keterangan';
...
var table;
    ...
    $(function () {
        table = $("#tagihan").DataTable({
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/notaris/tagihan') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/notaris/tagihan') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#tagihan_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Tagihan');
        ...
        .done((response) => {
            ...
            $('#modal-form [name=tgl_bayar]').attr('disabled', false);
            $('#modal-form [name=bulan_tagihan]').attr('disabled', false);
            $('#modal-form [name=tahun]').attr('disabled', false);
            ...
            $('#modal-form [name=notaris]').attr('disabled', false);
            $('#modal-form [name=total]').attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Tagihan');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=tgl_bayar]').val(response.tgl_bayar).attr('disabled', false);
            $('#modal-form [name=bulan_tagihan]').val(response.bulan_tagihan).attr('disabled', false);
            $('#modal-form [name=tahun]').val(response.tahun).attr('disabled', false);
            ...
            $('#modal-form [name=notaris]').val(response.notaris).attr('disabled', false);
            $('#modal-form [name=total]').val(response.total).attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Tagihan');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=tgl_bayar]').val(response.tgl_bayar).attr('disabled', true);
            $('#modal-form [name=bulan_tagihan]').val(response.bulan_tagihan).attr('disabled', true);
            $('#modal-form [name=tahun]').val(response.tahun).attr('disabled', true);
            ...
            $('#modal-form [name=notaris]').val(response.notaris).attr('disabled', true);
            $('#modal-form [name=total]').val(response.total).attr('disabled', true);
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Tagihan');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("tagihan.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
    ...
    function showImageModal(imageUrl) {
        var fullImageUrl = '{{ asset('storage/notaris/tagihan') }}/' + imageUrl;
