...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Notaris / Salinan</li>
@endsection
    ...
    <div class="card">
        <div class="card-body">
            ...
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            <button type="button" class="btn btn-success mb-2" data-toggle="modal" data-target="#salinan_modal"><i class="fas fa-file-download"></i> Import Excel</button>
            @endcan
            <table id="salinan" class="table table-bordered table-striped table-hover table-responsive salinan table-sm">
            ...
            <!-- Modal Import Excel -->
            <div class="modal fade" id="salinan_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form id="importForm" action="{{ route('salinan.import') }}" method="POST" enctype="multipart/form-data">
                        <div class="mt-3 mx-3">
                            <label for="file" class="control-label">Download Template Excel</label>
                            <a href="{{ route('salinan.export') }}" class="btn btn-secondary d-block"><i class="fas fa-file-upload"></i> Template Excel</a>
                        </div>
...
@includeIf('notaris.salinan.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#salinan").DataTable({
            ...
            columns: [
                ...
                //                 return '<img src="{{ asset('storage/notaris/salinan') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                //             } else {
                //                 // Jika file bukan gambar, tampilkan tombol download
                //                 return '<a href="{{ asset('storage/notaris/salinan') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                //             }
                //         }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#salinan_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Salinan');
        ...
        .done((response) => {
            ...
            $('#modal-form [name=tgl_realisasi]').attr('disabled', false);
            $('#modal-form [name=tgl_penyerahan]').attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Salinan');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=tgl_realisasi]').val(response.tgl_realisasi).attr('disabled', false);
            $('#modal-form [name=tgl_penyerahan]').val(response.tgl_penyerahan).attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Salinan');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=tgl_realisasi]').val(response.tgl_realisasi).attr('disabled', true);
            $('#modal-form [name=tgl_penyerahan]').val(response.tgl_penyerahan).attr('disabled', true);
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Salinan');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("salinan.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {
                    ...
                    // Tutup modal
                    $('#salinan_modal').modal('hide');

                    // Reset form
                    $('#importForm')[0].reset();

                    // Reload DataTable
                    $('#salinan').DataTable().ajax.reload();
                }
            },
            error: function (xhr) {
                // Jika gagal
    ...
    // var fullImageUrl = '{{ asset('storage/notaris/salinan') }}/' + imageUrl;
