...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Notaris / Minuta</li>
@endsection
    ...
    <div class="card">
        <div class="card-body">
            ...
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            <button type="button" class="btn btn-success mb-2" data-toggle="modal" data-target="#minuta_modal"><i class="fas fa-file-download"></i> Import Excel</button>
            @endcan
            <table id="minuta" class="table table-bordered table-striped table-hover table-responsive minuta table-sm">
            ...
            <!-- Modal Import Excel -->
            <div class="modal fade" id="minuta_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form id="importForm" action="{{ route('minuta.import') }}" method="POST" enctype="multipart/form-data">
                        <div class="mt-3 mx-3">
                            <label for="file" class="control-label">Download Template Excel</label>
                            <a href="{{ route('minuta.export') }}" class="btn btn-secondary d-block"><i class="fas fa-file-upload"></i> Template Excel</a>
                        </div>
...
var table;
    ...
    $(function () {
        table = $("#minuta").DataTable({
            ...
            columns: [
                ...
                //                 return '<img src="{{ asset('storage/notaris/minuta') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                //             } else {
                //                 // Jika file bukan gambar, tampilkan tombol download
                //                 return '<a href="{{ asset('storage/notaris/minuta') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                //             }
                //         }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#minuta_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Minuta');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Minuta');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Minuta');
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Minuta');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("minuta.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {
                    ...
                    // Tutup modal
                    $('#minuta_modal').modal('hide');

                    // Reset form
                    $('#importForm')[0].reset();

                    // Reload DataTable
                    $('#minuta').DataTable().ajax.reload();
                }
            },
            error: function (xhr) {
                // Jika gagal
    ...
    // var fullImageUrl = '{{ asset('storage/notaris/minuta') }}/' + imageUrl;
