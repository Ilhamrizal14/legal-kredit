...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Jaminan / Jaminan Masuk</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_jaminan_masuk')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            <button type="button" class="btn btn-success mb-2" data-toggle="modal" data-target="#jaminan_masuk_modal"><i class="fas fa-file-download"></i> Import Excel</button>
            @endcan
            <table id="jaminan_masuk" class="table table-bordered table-striped table-hover table-responsive jaminan_masuk table-sm">
            ...
            <!-- Modal Import Excel -->
            <div class="modal fade" id="jaminan_masuk_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form id="importForm" action="{{ route('jaminan_masuk.import') }}" method="POST" enctype="multipart/form-data">
                        <div class="mt-3 mx-3">
                            <label for="file" class="control-label">Download Template Excel</label>
                            <a href="{{ route('jaminan_masuk.export') }}" class="btn btn-secondary d-block"><i class="fas fa-file-upload"></i> Template Excel</a>
                        </div>
...
@includeIf('jaminan.jaminan_masuk.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#jaminan_masuk").DataTable({
            ...
            columns: [
                ...
                //                 return '<img src="{{ asset('storage/jaminan/jaminan_masuk') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                //             } else {
                //                 // Jika file bukan gambar, tampilkan tombol download
                //                 return '<a href="{{ asset('storage/jaminan/jaminan_masuk') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                //             }
                //         }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#jaminan_masuk_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Jaminan Masuk');
        ...
        $.get(url)
        .done((response) => {
            $('#modal-form [name=tgl_masuk]').attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Jaminan Masuk');
    ...
    $.get(url)
        .done((response) => {
            $('#modal-form [name=tgl_masuk]').val(response.tgl_masuk).attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Jaminan Masuk');
    ...
    $.get(url)
        .done((response) => {
            $('#modal-form [name=tgl_masuk]').val(response.tgl_masuk).attr('disabled', true);
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Jaminan Masuk');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("jaminan_masuk.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {
                    ...
                    // Tutup modal
                    $('#jaminan_masuk_modal').modal('hide');

                    // Reset form
                    $('#importForm')[0].reset();

                    // Reload DataTable
                    $('#jaminan_masuk').DataTable().ajax.reload();
                }
            },
            error: function (xhr) {
                // Jika gagal
    ...
    // var fullImageUrl = '{{ asset('storage/jaminan/jaminan_masuk') }}/' + imageUrl;
