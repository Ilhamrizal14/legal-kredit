...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Jaminan / Tukar Sementara</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_tukar_sementara')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            <button type="button" class="btn btn-success mb-2" data-toggle="modal" data-target="#tukar_sementara_modal"><i class="fas fa-file-download"></i> Import Excel</button>
            @endcan
            <table id="tukar_sementara" class="table table-bordered table-striped table-hover table-responsive tukar_sementara table-sm">
            ...
            <!-- Modal Import Excel -->
            <div class="modal fade" id="tukar_sementara_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form id="importForm" action="{{ route('tukar_sementara.import') }}" method="POST" enctype="multipart/form-data">
                        <div class="mt-3 mx-3">
                            <label for="file" class="control-label">Download Template Excel</label>
                            <a href="{{ route('tukar_sementara.export') }}" class="btn btn-secondary d-block"><i class="fas fa-file-upload"></i> Template Excel</a>
                        </div>
...
@includeIf('jaminan.tukar_sementara.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#tukar_sementara").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('tukar_sementara.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                { data: 'no_surat' },
                ...
                //                 return '<img src="{{ asset('storage/jaminan/tukar_sementara') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                //             } else {
                //                 // Jika file bukan gambar, tampilkan tombol download
                //                 return '<a href="{{ asset('storage/jaminan/tukar_sementara') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                //             }
                //         }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#tukar_sementara_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Tukar Sementara');
        ...
        $.get(url)
        .done((response) => {
            $('#modal-form [name=no_surat]').attr('disabled', false);
            $('#modal-form [name=nama]').attr('disabled', false);
            $('#modal-form [name=no_rekening]').attr('disabled', false);
            $('#modal-form [name=sebenarnya]').attr('disabled', false);
            $('#modal-form [name=yang_ditukar]').attr('disabled', false);
            $('#modal-form [name=tgl_kembali]').attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Tukar Sementara');
    ...
    $.get(url)
        .done((response) => {
            $('#modal-form [name=no_surat]').val(response.no_surat).attr('disabled', false);
            ...
            $('#modal-form [name=sebenarnya]').val(response.sebenarnya).attr('disabled', false);
            $('#modal-form [name=yang_ditukar]').val(response.yang_ditukar).attr('disabled', false);
            $('#modal-form [name=tgl_kembali]').val(response.tgl_kembali).attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Tukar Sementara');
    ...
    $.get(url)
        .done((response) => {
            $('#modal-form [name=no_surat]').val(response.no_surat).attr('disabled', true);
            ...
            $('#modal-form [name=sebenarnya]').val(response.sebenarnya).attr('disabled', true);
            $('#modal-form [name=yang_ditukar]').val(response.yang_ditukar).attr('disabled', true);
            $('#modal-form [name=tgl_kembali]').val(response.tgl_kembali).attr('disabled', true);
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Tukar Sementara');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("tukar_sementara.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status === 'success') {
                    ...
                    // Tutup modal
                    $('#tukar_sementara_modal').modal('hide');

                    // Reset form
                    $('#importForm')[0].reset();

                    // Reload DataTable
                    $('#tukar_sementara').DataTable().ajax.reload();
                }
            },
            error: function (xhr) {
                // Jika gagal
    ...
    // var fullImageUrl = '{{ asset('storage/jaminan/tukar_sementara') }}/' + imageUrl;
